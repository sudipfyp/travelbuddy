...
def sendVerificationEmail(email):
    ...
    if len(userData) > 0:
         Code.objects.create(code=otp, user_id=userData[0].id, expiry=five_minutes_later)
    elif len(guideData) > 0:
         Code.objects.create(code=otp, guide_id=guideData[0].id, expiry=five_minutes_later)
            
    elif len(sellerdata) > 0:
        Code.objects.create(code=otp, seller_id=sellerdata[0].id, expiry=five_minutes_later)
