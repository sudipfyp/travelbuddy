        ...
        if verification:
            if payload['role'].lower() == "admin":
                ...
                if serailizer.is_valid():
                    ...
                    Place.objects.create(name=name, location=location, district=district, latitude=latitude,
                                         longitude=longitude, tag=tag, description=description, image=image)
        ...
        if verification:
            if payload['role'].lower() == "admin":
                ...
                if serailizer.is_valid():
                    ...
                    placeObj.update(name=name, location=location, district=district,
                                    latitude=latitude, longitude=longitude, tag=tag, description=description)

                    placeobject = Place.objects.get(id=kwargs['id'])
                    if image:
                        placeobject.image = image
                        placeobject.save()
...
class Recommendation(APIView):
    def post(self, request):
        ...
        limitValue = int(days) * 2
        places = Place.objects.filter(tag=tag)[:limitValue]
        if len(places) == 0:
            return Response({"msg": "Not Found"}, status=status.HTTP_404_NOT_FOUND)
